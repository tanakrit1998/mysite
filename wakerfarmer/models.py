...
class Queue(models.Model):
    ...
    def __str__(self):
        return f'{self.qid} - คิวที่ {self.queue}'
...
class Price(models.Model):
    ...
    def __str__(self):
        return f'{self.pid} - ราคาข้าวหอมมะลิ {self.price} -ราคาข้าวเหนียว {self.sprice}'
...
class Map(models.Model):
    ...
    def __str__(self):
        return f'{self.lid}  ละติจูด {self.lat}  ลองจิจูด {self.lng}'

class Mill(models.Model):
    mid    = models.AutoField(primary_key = True)
    queue  = models.ForeignKey(Queue,on_delete=models.CASCADE,default=1)
    name   = models.CharField(max_length  = 100)
    price  = models.ForeignKey(Price,on_delete=models.CASCADE,default=1)
    map    = models.ForeignKey(Map,on_delete=models.CASCADE,default=1)
 
    def __str__(self):
        return f'{self.mid} - โรงสี - {self.name} - คิวที่ - {self.queue}  {self.price} - ละติจูด ลองจิจูด - {self.map} '
